...
static int xgmac_mdio_read(struct mii_bus *bus, int addr, int regnum)
{
	...
	if (regnum & MII_ADDR_C45) {
		...
		reg |= FIELD_PREP(MII_XGMAC_DA, FIELD_GET(MII_DEVADDR_C45_MASK, regnum)) |
		       FIELD_PREP(MII_XGMAC_RA, FIELD_GET(MII_REGADDR_C45_MASK, regnum));
	} else {
...
static int xgmac_mdio_init(struct xgmac_priv *priv)
{
	...
	priv->mii->probe_capabilities = MDIOBUS_C22_C45;
...
MODULE_DESCRIPTION("Ethernet XGMAC driver for SoC");
