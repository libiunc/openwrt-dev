		      ...
		      u32 rdes0, u32 rdes_ctx1)
{
	...
	sf_dbg_ratelimited("Hash is 0x%04x\n", hash);
				...
				struct xgmac_rxq *rxq)
{
	...
	for (; dirty > 0; dirty -= 2) {
		...
		xgmac_dma_init_rx_desc(p, BUF_ADDR_ALIGN(page_pool_get_dma_addr(buf->page) + buf->offset + BUF_PAD), false);
...
static int xgmac_dma_poll_rx(struct xgmac_rxq *rxq, int budget)
{
	...
	for (; count < budget; count++) {
		...
		if (id >= DPNS_MAX_PORT) {
			sf_dbg_ratelimited("error ivport:%u\n", id);
			continue;
		}
		...
		dma_sync_single_for_cpu(priv->dev, BUF_ADDR_ALIGN(page_pool_get_dma_addr(buf->page) + buf->offset + BUF_PAD), len, DMA_FROM_DEVICE);
		net_prefetch(page_address(buf->page) + buf->offset + BUF_PAD);
		...
		sf_dbg_ratelimited("%s: up_reason:%02x sta_index:%u rpt_index:%u "
				"ovid:%u vlan_pri:%u no_frag:%u sport:%u eth_type:0x%x "
				"dscp:%u tnp:%u pkt_type:%u smac:%llx\n",
				netdev->name, up_reason, skb->sta_index, skb->rpt_index,
...
static int xgmac_dma_init_rx_rings(struct xgmac_dma_priv *priv)
{
	...
	for (queue = 0; queue < DMA_CH_MAX - 1; queue++) {
...
#ifdef CONFIG_NET_SIFLOWER_ETH_USE_INTERNAL_SRAM
		txq->dma_tx = gen_pool_dma_alloc(priv->genpool,
				DMA_TX_SIZE * sizeof(*txq->dma_tx),
				&txq->dma_tx_phy);
#else
		if (i == DMA_DESC_TX_SRAM_CHANNEL) {
			...
			txq->dma_tx = dma_alloc_coherent(priv->dev,
					DMA_TX_SIZE * sizeof(*txq->dma_tx),
...
static netdev_tx_t xgmac_dma_xmit_direct(struct sk_buff *skb, struct xgmac_dma_priv *priv)
{
...
DEFINE_PER_CPU_ALIGNED(struct wifi_to_dma, wifi_to_dma);
EXPORT_PER_CPU_SYMBOL(wifi_to_dma);
struct wifi_to_dma {
...
void dump_wifi_to_dma_data(bool is_clear)
{
	struct wifi_to_dma *wtd;
	int i;
	if (is_clear) {
		for (i = 0; i < NR_CPUS - 1; i++) {
			wtd = per_cpu_ptr(&wifi_to_dma, i);
			wtd->send_cnt = 0;
			wtd->drop_cnt = 0;
			wtd->recv_cnt = 0;
		}
		printk("clear all wifi count\n");
	} else {
		for (i = 0; i < NR_CPUS - 1; i++) {
			wtd = per_cpu_ptr(&wifi_to_dma, i);
			printk("CPU%d recv: %u\t send: %u\t drop: %u\n",
				wtd->cpu, wtd->recv_cnt, wtd->send_cnt, wtd->drop_cnt);
		}
	}
}
EXPORT_SYMBOL(dump_wifi_to_dma_data);

static void wtd_lock(struct wifi_to_dma *wtd) {
	spin_lock(&wtd->input_pkt_queue.lock);
}

static void wtd_unlock(struct wifi_to_dma *wtd) {
	spin_unlock(&wtd->input_pkt_queue.lock);
}

static void dma_tasklet_function(unsigned long data)
{
	struct wifi_to_dma *wtd = (struct wifi_to_dma *)data;
	bool again = true;

	wtd_lock(wtd);
	wtd->status = STATE_BUSY;
	skb_queue_splice_tail_init(&wtd->input_pkt_queue,
				   &wtd->process_queue);
	wtd_unlock(wtd);
	...
	while(again) {
		...
		while((skb = __skb_dequeue(&wtd->process_queue))) {
			if (xgmac_dma_xmit_direct(skb, g_priv))
				wtd->drop_cnt++;
			else
				wtd->send_cnt++;
		}
		...
		wtd_lock(wtd);
		if (skb_queue_empty(&wtd->input_pkt_queue)) {
			again = false;
...
static void csd_func(void *data)
{
	struct wifi_to_dma *wtd = (struct wifi_to_dma *)data;

	tasklet_schedule(&wtd->dma_tasklet);
}
...
static int xgmac_search_ndev_index(struct net_device *dev)
{
	struct xgmac_dma_priv *priv = g_priv;
	int i;

	for (i = EXTDEV_OFFSET; i < DPNS_MAX_PORT; i++) {
		if (dev == priv->ndevs[i])
			break;
	}

	if (i == DPNS_MAX_PORT)
		printk("ERROR: net device not found\n");

	return i;
}
EXPORT_SYMBOL(xgmac_search_ndev_index);

int wifi_xgmac_dma_xmit(struct sk_buff *skb, u16 hash, u8 index)
{
	struct wifi_to_dma *wtd;
	struct xgmac_dma_priv *priv = g_priv;
	struct xgmac_skb_cb *cb = XGMAC_SKB_CB(skb);
	u32 cpu = hash % (NR_CPUS - 1);
	u32 queue = hash % NR_CPUS;
	bool flag = false;

	if (skb->dev != priv->ndevs[index])
		index = xgmac_search_ndev_index(skb->dev);

	if (index == DPNS_MAX_PORT)
		return -EINVAL;
	...
	/* txq[4] is dma channel 4 for game port
	 * so if skb form cpu 2/3 should use txq[0]/[1], skb from game port vap use txq[2]
	 */
		if (g_priv->ifindex == skb->dev->ifindex)
			skb_set_queue_mapping(skb, DMA_OVPORT_CH);
		else
			skb_set_queue_mapping(skb, cpu);
	}
	__skb_queue_tail(&wtd->input_pkt_queue, skb);
	if (wtd->status == STATE_IDLE)
		flag = true;
	wtd_unlock(wtd);

	if (cpu_online(cpu) && flag) {
		smp_call_function_single_async(cpu, &wtd->csd);
	}

	return 0;
}
EXPORT_SYMBOL(wifi_xgmac_dma_xmit);
...
static int xgmac_tx_stats_show(struct seq_file *m, void *v)
{
	dump_wifi_to_dma_data(0);
	dump_wifi_to_dma_data(1);

	return 0;
}

DEFINE_SHOW_ATTRIBUTE(xgmac_tx_stats);
...
static int xgmac_dma_probe(struct platform_device *pdev)
{
	...
	/* default use mapping mode 1 */
	ret = regmap_write(priv->ethsys, ETHSYS_MRI_Q_EN, 0x0000001F);
	...
	for (i = 0; i < DMA_CH_MAX - 1; i++) {
		...
		netif_tx_napi_add(&priv->napi_dev, &priv->txq[i].napi,
			xgmac_dma_napi_tx, NAPI_POLL_WEIGHT);
		/* txq0->cpu0
		 * txq1->cpu1
		 * txq2->cpu2
		 * txq3->cpu3
		 * if close the macro txq3->cpu1, cpu3 reserved for wifi
		 */
	...
	for (i = 0; i < DMA_CH_MAX - 1; i++) {
		 ...
		 * rxq2->cpu2
		 * rxq3->cpu3
		 * if close the macro rxq3->cpu1, cpu3 reserved for wifi
		 */
#ifdef CONFIG_DPNS_THROUGHPUT_ETH_BEST
		cpu = i;
#else
	...
	sf_dynamic_debug_init(THIS_MODULE);
...
static int xgmac_dma_remove(struct platform_device *pdev)
{
	...
	sf_dynamic_debug_deinit(THIS_MODULE);

	return 0;
}

static const struct of_device_id xgmac_dma_match[] = {
	{ .compatible = "siflower,xgmac-dma" },
	{},
};
...
MODULE_DESCRIPTION("Ethernet DMA driver for SoC");
