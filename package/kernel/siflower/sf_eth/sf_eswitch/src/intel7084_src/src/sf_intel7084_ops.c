...
int intel7084_get_port_link(struct switch_dev *dev,  int port,
...
static int intel_rgmii_init(int port)
{
	...
	if (s != 0)
		return s;
	if (port == 5) {
		s = intel7084_mdio_wr(MII_CFG_5_EN_OFFSET, MII_CFG_5_EN_SHIFT,
				      MII_CFG_5_EN_SIZE, 1);
		if (s != 0)
			return s;
		s = intel7084_mdio_wr(MII_CFG_5_RGMII_IBS_OFFSET, MII_CFG_5_RGMII_IBS_SHIFT,
				      MII_CFG_5_RGMII_IBS_SIZE, 1);
	} else if (port == 6) {
		s = intel7084_mdio_wr(MII_CFG_6_EN_OFFSET, MII_CFG_6_EN_SHIFT,
				      MII_CFG_6_EN_SIZE, 1);
		if (s != 0)
			return s;
		s = intel7084_mdio_wr(MII_CFG_6_RGMII_IBS_OFFSET, MII_CFG_6_RGMII_IBS_SHIFT,
				      MII_CFG_6_RGMII_IBS_SIZE, 1);
	}
	printk("End %s\n", __func__);
	return s;
}
...
void intel7084_init(struct sf_eswitch_priv *pesw_priv)
{
	...
	intel7084_mdio_wr(MII_CFG_6_EN_OFFSET, MII_CFG_6_EN_SHIFT,
			  MII_CFG_6_EN_SIZE, 0);

	intel_rgmii_init(5);
	if(pesw_priv->model == INTEL7082) {
		intel_port_rgmii_dalay_set(5, 4, 0);
	}
#ifdef CONFIG_SF19A28_MPW1
...
static int intel7084_set_rgmii(bool enable)
{
	int s;

	s = intel7084_mdio_wr(MII_CFG_5_RGMII_IBS_OFFSET, MII_CFG_5_RGMII_IBS_SHIFT,
			      MII_CFG_5_RGMII_IBS_SIZE, enable);
	if (s)
		return s;
	msleep(100);
	s = intel7084_mdio_wr(MII_CFG_5_EN_OFFSET, MII_CFG_5_EN_SHIFT,
			      MII_CFG_5_EN_SIZE, enable);
	msleep(100);

	return s;
}

static int intel7084_irq_init(int type)
{
	int s = 0, i;
	...
	case IRQ_TYPE_LEVEL_HIGH:
		s = intel7084_mdio_wr(EIU_EXIN_CONF_EOUT0_OFFSET, EIU_EXIN_CONF_EOUT0_SHIFT,
				      EIU_EXIN_CONF_EOUT0_SIZE, 0);
		break;
	case IRQ_TYPE_EDGE_FALLING:
	case IRQ_TYPE_LEVEL_LOW:
		s = intel7084_mdio_wr(EIU_EXIN_CONF_EOUT0_OFFSET, EIU_EXIN_CONF_EOUT0_SHIFT,
				      EIU_EXIN_CONF_EOUT0_SIZE, 1);
		break;
	default:
		s = -EINVAL;
	}
	...
	// enable GSWIP MAC interrupt
	s = intel7084_mdio_wr(ETHSW_IER_MACIE_OFFSET, ETHSW_IER_MACIE_SHIFT,
			      ETHSW_IER_MACIE_SIZE, 1);
	if (s)
		return s;
	// enable port 0~4 interrupt
	s = intel7084_mdio_wr(MAC_IER_MACIEN_OFFSET, MAC_IER_MACIEN_SHIFT,
			      MAC_IER_MACIEN_SIZE, 0x1f);
	if (s)
		return s;

	for (i = 0; i < INTEL_PHY_PORT_NUM; i++) {
		// enable per-port link change interrupt
		s = intel7084_mdio_wr(MAC_PIER_PACT_OFFSET + i * 0xc, MAC_PIER_PACT_SHIFT,
				      MAC_PIER_PACT_SIZE, 1);
		if (s)
			return s;
	}

	// set GPIO0(MMDIO) to EXINT mode
	s = intel7084_mdio_wr(GPIO_ALTSEL0_GPIO_OFFSET, 0, 1, 0);
	if (s)
		return s;
	s = intel7084_mdio_wr(GPIO_ALTSEL1_GPIO_OFFSET, 0, 1, 1);
	if (s)
		return s;
	// enable GSWIP interrupt for EXINT0
	s = intel7084_mdio_wr(IM0_EINT0_IER_IR5_OFFSET, IM0_EINT0_IER_IR5_SHIFT,
			      IM0_EINT0_IER_IR5_SIZE, 1);
	if (s)
		return s;
	// GPIO0 direction output
	s = intel7084_mdio_wr(GPIO_DIR_GPIO_OFFSET, 0, 1, 1);
	return s;
}

static int intel7084_irq_ack(void)
{
	int s, val, i;

	s = intel7084_mdio_rd(MAC_ISR_MACINT_OFFSET, MAC_ISR_MACINT_SHIFT,
		              MAC_ISR_MACINT_SIZE, &val);
	if (s)
		return s;
	...
	for (i = 0; i < INTEL_PHY_PORT_NUM; i++) {
		...
		// ack interrupt
		s = intel7084_mdio_wr(MAC_PISR_PACT_OFFSET + i * 0xc, 0, 16, 0x2000);
		if (s)
			return s;
	}

	s = intel7084_mdio_wr(IM0_ISR_IR0_OFFSET, 0, 16, 0x20);
	if (s)
		return s;
...
struct sf_eswitch_api_t intel7084_api = {
	...
	.set_rgmii = intel7084_set_rgmii,
	.irq_init = intel7084_irq_init,
	.irq_ack = intel7084_irq_ack,
};
